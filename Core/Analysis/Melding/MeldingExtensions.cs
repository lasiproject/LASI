...
namespace LASI.Core
{
    public static class MeldingExtensions
    {
        public static IEnumerable<IEntity> Meld<TEntity>(this IEnumerable<TEntity> entities) where TEntity : class, IEntity {
            return MeldImplementation(entities, CustomComparer.Create((TEntity x, TEntity y) => x.IsSimilarTo(y)));
        }
        public static IEnumerable<IEntity> Meld<TEntity>(this IEnumerable<TEntity> entities, Func<TEntity, TEntity, bool> meldOn) where TEntity : class, IEntity {
            return MeldImplementation(entities, CustomComparer.Create(meldOn));
        }
        private static IEnumerable<IEntity> MeldImplementation<TEntity>(IEnumerable<TEntity> entities, IEqualityComparer<TEntity> entityComparer)
            ...
            var result = from toMeld in groupsToMeld
                         let avatar = toMeld.GroupBy(entity => entity.Text)
                                .MaxBy(g => g.Count())
                                .First()
