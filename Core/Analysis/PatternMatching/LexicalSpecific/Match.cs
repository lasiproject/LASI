...
{
    /// <summary> Provides for the representation and free form structuring of Type based Pattern
    /// Matching expressions which match with a value of Type T and does not yield a result.
    /// </summary> <typeparam name="T"> The Type of the value which the the Pattern Matching
    /// expression will match with. </typeparam> <remarks> <para> It is important to note that null
    /// values are coalesced by the Match process. That is to say that if the value being tested is
    /// null it will never match any of the Clauses. Because of this a null value will always yield
    /// the default result and never produce an error. </para> <para> The type based pattern
    /// matching functionality was introduced to solve the problem of performing subtype dependent
    /// operations which could not be described by traditional virtual method approaches in an
    /// expressive or practical manner. There are several reasons for this. First and most
    /// important, is that the virtual method approach is limited to single dispatch* . Secondly
    /// different binding operations must be chosen based on a variety of information gathered from
    /// contexts which are often external to the single instance of the type representing a lexical
    /// construct. Different numbers and types of arguments that depend on the surrounding context
    /// of the lexical instnace cannot be specified in a manner compatible with virtual method
    /// signatures. However, a linear storage and traversal mechanism for lexical elements of
    /// differing syntactic types, as well as the need to define types representing elements having
    /// overlapping syntactic roles, remains a necessity which prevents the complete stratification
    /// of class and interface hierarchies. There were a variety of solutions that were considered:
    /// </para> <para>
    ...
    ///    generally increasing complexity. </para> <para>
    ...
    ///    control flow, never stating it explicitly. </para> <para>
    ...
    ///    prone to subtle errors**. </para> <para> The pattern matching implemented here abstracts
    /// over the type checking logic, allowing it to be tested and optimized for the large variety
    /// of algorithms in need of such functionality, allows for subtype constraints to be specified,
    /// allows for algorithms to handle as many or as few types as needed, and emphasizes the intent
    /// of the code which leverage it. It also eliminates the difficulty of defining state-full
    /// visitors by defining a match with a particular subtype as a function on that type. Such a
    /// function is intuitively specified as an anonymous closure which implicitly captures any
    /// state it will use. This approach also encourages the localization of logic, which in the
    /// case of visitors would not possible as implementations of visitors will inherently get
    /// spread out in both the textual space of the source code and the conceptions of implementers.
    /// The syntax for pattern matching uses a fluent interface style. </para> <example> <code> var
    /// weight = myLexical.Match() .Case((IReferencer r) =&gt;
    /// Console.WriteLine(r.ReferredTo.Weight)) .Case((IEntity e) =&gt; Console.WriteLine(e.Weight))
    /// .Case((IVerbal v) =&gt; Console.WriteLine(v.HasSubject()? v.Subject.Weight : 0)); </code>
    /// </example><example> <code> var weight = myLexical.Match() .Case((Phrase p) =&gt;
    /// Console.WriteLine(p.Words.Average(w =&gt; w.Weight))) .Case((Word w) =&gt;
    /// Console.WriteLine(w.Weight)) .Default(()=&gt; Console.WriteLine("not a word or phrase"))
    /// </code> </example> <para> Patterns may be nested arbitrarily as in the following example
    /// </para> <para> When a Yield clause is not applied, the Match Expression will not yield a
    /// value. Instead it will behave much as a Type driven switch statement. <example> <code>
    /// myLexical.Match() .Case((Phrase p) =&gt; Console.WriteLine("Phrase: ", p.Text)) .Case((Word
    /// w) =&gt; Console.WriteLine("Word: ", w.Text)) .Default(() =&gt; Console.WriteLine("Not a
    /// Word or Phrase")); </code> </example> </para> <para>
    ...
    /// readability and maintainability. </para> <para>
    ...
    /// conversions are defined) </para> </remarks> <seealso cref="LASI.Core.MatchExtensions">
    /// Provides extension methods which allow for the creation of Match expressions. </see>
    [DebuggerStepThrough]
    public class Match<T> : MatchBase<T> where T : class, ILexical
    {
        ...
        /// returning expression of Case&lt;T, R&gt; Such that it will yield a result value of the
