...
require('../../js/tab.js')
require('../../js/affix.js')
(function(root){var opts={mains:[],base:''};var basedUrl=function(path){return opts.base+'/'+path;};root.bowerRequireJsAuto=function(){opts=getOpts();var url=basedUrl('bower.json');get(url,function(xhr){var config=JSON.parse(xhr.responseText);handleConfig(config);});};var getOpts=function(){var scripts=getScripts();var script;for(var i=0,m=scripts.length;i<m;++i){script=scripts[i];var then=script.attributes['data-then'];if(then){var thenVal=then.value;opts.mains.push(thenVal);}
var base=script.attributes['data-base'];if(base){var baseVal=base.value;opts.base=baseVal;}}
return opts;};var loadMain=function(){var mains=opts.mains;if(mains){for(var i=0,m=mains.length;i<m;++i){var main=mains[i];require([main]);}}};var getScripts=function(){return document.getElementsByTagName('script');};var get=function(url,done,opts){opts=opts||{};var xhr=new XMLHttpRequest();xhr.open('GET',url,false);xhr.onreadystatechange=function(){if(xhr.readyState===4&&xhr.status<400){done(xhr);}};xhr.send(null);};var handleConfig=function(config){var deps=config.dependencies;if(deps){var i=0;var count=0;var depName;for(depName in deps){++count;}
var eachDepCallback=function(){++i;if(i==count){loadMain();}};var dep;for(depName in deps){dep=deps[depName];eachDep(dep,depName,eachDepCallback);}}};var isArray=function(val){return Object.prototype.toString.call(val)==='[object Array]';};var toArray=function(main){if(isArray(main)){return main;}
else{return[main];}};var eachDep=function(dep,depName,callback){var dir=basedUrl('bower_components/'+depName);get(dir+'/.bower.json',function(xhr){var config=JSON.parse(xhr.responseText);var main=config.main;if(!main){main='index.js';}
var mainItems=toArray(main);var mainItem;for(var i=0,m=mainItems.length;i<m;++i){mainItem=mainItems[i];if(!mainItem.match(/\.js$/)){continue;}
mainItem=mainItem.replace(/\.js$/,'');var path=dir+'/'+mainItem;var paths={};paths[depName]=path;var requireConfig={paths:paths};require.config(requireConfig);callback();}});};root.bowerRequireJsAuto();})(this);
...
(function(QUnit,env){if(env.__quit_once_initialized){return;}
env.__quit_once_initialized=true;if(typeof QUnit!=='object'){throw new Error('undefined QUnit object');}
var _module=QUnit.module;if(typeof _module!=='function'){throw new Error('QUnit.module should be a function');}
QUnit.module=function(name,config){if(typeof config!=='object'){return _module.call(QUnit,name,config);}
(function addSetupOnce(){if(QUnit.supports&&QUnit.supports.setupOnce){return;}
if(typeof config.setupOnce==='function'){var _setupOnceRan=false;var _setup=typeof config.setup==='function'?config.setup:null;config.setup=function(){if(!_setupOnceRan){config.setupOnce();_setupOnceRan=true;}
if(_setup){_setup.call(config);}};}}());(function addTeardownOnce(){if(QUnit.supports&&QUnit.supports.teardownOnce){return;}
function isLastTestInModule(){if(QUnit.config&&Array.isArray(QUnit.config.queue)){return QUnit.config.queue.length===1;}else{return false;}}
if(typeof config.teardownOnce==='function'){var _teardown=typeof config.teardown==='function'?config.teardown:null;config.teardown=function(){if(_teardown){_teardown.call(config);}
if(isLastTestInModule()){config.teardownOnce();config.teardownOnceRan=true;}};QUnit.moduleDone(function(details){if(details.name===name){if(!config.teardownOnceRan){config.teardownOnce();config.teardownOnceRan=true;}}});}}());_module.call(QUnit,name,config);};}(QUnit,typeof global==='object'?global:window));
