namespace LASI.Core.Analysis.PatternMatching.LexicalSpecific.Experimental.FromSpec
{
    class ExactTextPredicate<T> : Predicate<T> where T : class, ILexical
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ExactTextPredicate{T}"/> class.
        /// </summary>
        /// <param name="text">The text which an element must exactly match to satisfy the predicate.</param>
        public ExactTextPredicate(string text) => this.text = text;

        /// <summary>
        /// Returns a value indicating if the given <see cref="ILexical"/> instance's textual content exactly matches the value from which the predicate was constructed <see cref="Predicate{T}"/>.
        /// </summary>
