...
namespace LASI.Core.Heuristics.WordNet
{
    ...
    {
        protected Synset(int id, IEnumerable<string> words,
            IEnumerable<KeyValuePair<TLinkKind, int>> pointerRelationships)
        {
            Id = id;
            Words = new HashSet<string>(words, StringComparer.OrdinalIgnoreCase);
            referencedSetsByLinkType =
                new Lazy<ILookup<TLinkKind, int>>(() => pointerRelationships.ToLookup(p => p.Key, p => p.Value));
        ...
        public IEnumerable<int> this[params TLinkKind[] linkKinds] =>
        linkKinds.SelectMany(link => RelatedSetIdsByRelationKind[link]);

        private readonly Lazy<ILookup<TLinkKind, int>> referencedSetsByLinkType;
        ...
        /// <summary>
        /// Returns a value indicating whether the current Synset directly links to the given <see cref="Synset{TLinkKind}"/>.
        /// </summary>
        /// <param name="other">The Synset to find.</param>
        /// <returns><c>true</c> if the current Synset directly links to given <see cref="Synset{TLinkKind}"/>; otherwise false.</returns>
        public bool DirectlyReferences(Synset<TLinkKind> other) => ReferencedSets.Contains(other.Id);

        public sealed override int GetHashCode() => Id;

        public bool Equals(Synset<TLinkKind> other) => Id == other.Id;

        public sealed override bool Equals(object obj) => obj is Synset<TLinkKind> s && Equals(s);

        /// <summary>
        /// Returns a single string representing the <see cref="Synset{TLinkKind}"/>.
        /// </summary>
        /// <returns>A single string representing the <see cref="Synset{TLinkKind}"/>.</returns>
        public sealed override string ToString() => "[" + Id + "] " + Words.Format((' ', ',', ' '));
    ...
    {
        public NounSynset(int id, IEnumerable<string> words,
            IEnumerable<KeyValuePair<NounLink, int>> pointerRelationships, NounCategory category) : base(id, words, pointerRelationships) => Category = category;
    ...
    {
        public VerbSynset(int id, IEnumerable<string> words, IEnumerable<KeyValuePair<VerbLink, int>> referencedSets,
            VerbCategory category) : base(id, words, referencedSets) => Category = category;
    ...
    {
        public AdjectiveSynset(int id, IEnumerable<string> words,
            IEnumerable<KeyValuePair<AdjectiveLink, int>> pointerRelationships, AdjectiveCategory category) : base(id, words, pointerRelationships) => Category = category;
    ...
    {
        public AdverbSynset(int id, IEnumerable<string> words,
            IEnumerable<KeyValuePair<AdverbLink, int>> pointerRelationships, AdverbCategory category) : base(id, words, pointerRelationships) => Category = category;
