...
namespace LASI.Utilities
{
    ...
    {
        /// <summary>
        /// Creates an <see cref="IEqualityComparer{T}"/> which will use the provided function to determine equality, and define hash codes purely in terms of nullity.
        /// </summary>
        /// <typeparam name="T">The type of the objects to compare.</typeparam>
        /// <param name="equals">A function which determines if two objects of type T are equal.</param>
        /// <exception cref="ArgumentNullException">The provided <paramref name="equals"/> function is null.</exception>
        /// <returns>An <see cref="IEqualityComparer{T}"/> which will use the provided function to determine equality, and defines hash codes purely in terms of nullity.</returns>
        /// <remarks>
        /// <para>
        /// The intent of the functionality provided is to simplify the use of LINQ methods, such as
        /// <see cref="Enumerable.Contains{TSource}(IEnumerable{TSource}, TSource, IEqualityComparer{TSource})"/> and
        /// <see cref="Enumerable.Distinct{TSource}(IEnumerable{TSource}, IEqualityComparer{TSource})"/> which require an IEqualityComparer as opposed to a simple predicate function. Because the custom
        /// comparer created produces identical hashcodes for non null objects, it allows for these methods to behave more transparently, but introduced. This however means that the overhead of using
        /// the returned comparer in a LINQ query is substantial. Specifically, the complexity of a calling methods such as IEnumerable&lt;T&gt;.Contains(item, (x, y) =&gt; x == y) and
        /// IEnumerable&lt;T&gt;.Distinct((x, y) =&gt; x == y) approaches O(N^2). Comparatively, calls which use the default equality comparers, including those that do not take an
        ...
        /// <summary>
        /// Creates a new <see cref="IEqualityComparer{T}"/> which will use the provided equality and hashing functions.
        /// </summary>
        /// <param name="equals">     A function which determines if two objects of type T are equal.</param>
        /// <param name="getHashCode">A function which generates a hash code from an element of type T.</param>
        /// <exception cref="ArgumentNullException">The provided <paramref name="equals"/> or <paramref name="getHashCode"/> function is null.</exception>
        /// <returns>A new <see cref="IEqualityComparer{T}"/> which will use the provided equality and hashing functions.</returns>
        /// <remarks>
        /// Proper usage requires that elements which will compare equal under the specified equals function will also produce identical hashcodes. Elements may yield identical hash codes, without
        ...
        {
            /// <summary>
            /// Initializes a new instance of the <see cref="ComparerWithCutomEqualsAndNullityBasedHashing{T}"/> class which will use the provided function to define element equality.
            /// </summary>
            /// <param name="equals">A function which determines if two objects of type T are equal.</param>
            /// <exception cref="ArgumentNullException">The provided <paramref name="equals"/> function is null.</exception>
            /// <remarks>
            /// A custom hashing function is automatically provided, ensuring that equality comparisons take place except when reference is null. While this provides clean, customizable semantics for
            /// set operations, more expensive to use having a complexity of N^2
            /// </remarks>
            public ComparerWithCutomEqualsAndNullityBasedHashing(Func<T, T, bool> equals) => this.equals = equals;
        ...
        /// <remarks>
        /// <para>
        /// The primary purpose of this class is to allow for the ad hoc, inline creation of an IEqualityComparer&lt;T&gt; from arbitrary functions. This allows for the easy use of Query Operators
        /// taking an IEqualityComparer&lt;T&gt; as an argument.
        /// </para>
        /// <para>
        /// Proper usage requires that elements which will compare equal under the specified equals function will also produce identical hashcodes. Elements may yield identical hash codes, without
        /// being considered equal.
