...
var GetInfoForVerbal = (rawData: string): VerbalContextInfo => {
    var dataSource: any = $.parseJSON(rawData);
    return dataSource;
}

$(() => {
    var showApplicableMenuItems = (controls: JQuery, relatedElementIds: number[], elementAction) => {
    ...
    verbals.on("contextmenu", function (event) {
        var verbalInfo: VerbalContextInfo;
