...
(function (LASI) {
    ...
    (function (results) {
        'use strict';
        var highlightRelatedElement = function (element, index) { return $(element).addClass('selected'); }, verbals = $('.lexical-content-block span.verbal'), referencers = $('.lexical-content-block span.referencer'), contextMenu = $('div#contextMenu'), displayContextMenuForElement = function (element, event) { return element.css({
            ...
            controls
                .each(function (index, element) { return relatedElementIds ? $(element).show() : $(element).hide(); })
                .click(function (event) {
                event.preventDefault();
                (relatedElementIds || [])
                    .map(function (element, index) { return $('#' + element.toString()); })
            ...
            if (verbalContext.directObjects && verbalContext.directObjects.length ||
                verbalContext.indirectObjects && verbalContext.indirectObjects.length ||
                verbalContext.subjects && verbalContext.subjects.length) {
